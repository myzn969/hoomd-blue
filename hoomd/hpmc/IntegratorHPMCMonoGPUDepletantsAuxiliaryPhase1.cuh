...
#include "IntegratorHPMCMonoGPUDepletantsAuxiliaryPhase1.inc"
    ...
    if (max_threads == cur_launch_bounds*MIN_BLOCK_SIZE)
        ...
        if (max_block_size == -1)
            ...
            hipFuncGetAttributes(&attr,
                reinterpret_cast<const void*>(&kernel::hpmc_insert_depletants_phase1<Shape, launch_bounds_nonzero*MIN_BLOCK_SIZE>));
        ...
        for (int idev = auxiliary_args.gpu_partition_rank.getNumActiveGPUs() - 1; idev >= 0; --idev)
            ...
            hipLaunchKernelGGL((kernel::hpmc_insert_depletants_phase1<Shape, launch_bounds_nonzero*MIN_BLOCK_SIZE>),
...
//! Entry point for precompiled kernel::insert_depletants_phase1() without patch interactions
/*! \param args Bundled arguments
    \param implicit_args Bundled arguments related to depletants
    \param auxiliary_args Bundled arguments for auxillary variable depletants
